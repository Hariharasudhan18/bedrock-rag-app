...
import json
import os
# from utils.claude import ClaudeSonnet
...
# Memory buffer to maintain chat history using LangChain
...
# Function to generate response with LangChain
def generate_response_with_langchain(context, question):
...
def main():
   ...
   user_query = "What is the main GenAI Hub topic discussed in the document?"
   ...
   context = " ".join([text_chunks[idx] for idx in indices[0]])
   chat_history.append(user_query)
   #prompt_with_history = "\n".join(chat_history) + "\n" + prompt

   # Step 10: Generate response using LLM
   # generated_response = generate_response(prompt_with_history)
   generated_response = generate_response_with_langchain(context, user_query)

   # Step 11: Apply guardrails to the response
   # safe_response = apply_guardrails(generated_response)

   # context_embedding = get_embeddings(context[:2048])
   # response_embedding = get_embeddings(safe_response[:2048])

   # # Step 12: Evaluate the response
   # evaluation_score = evaluate_responses(context_embedding, response_embedding)
   # print(f"Evaluation Score (Cosine Similarity): {evaluation_score}")
   # print("type of generated response: ", type(generated_response))
   # print("len of generated response: ", len(generated_response))
   # Display the generated response
   print(f"Generated Response App3 Langchain: ")
